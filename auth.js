...
//
// ■ PIN の変更方法（数字4桁）:
//   1. ブラウザの console（F12）で以下を実行してハッシュを取得:
//      const p = '新しい4桁';
...
//   2. 出力された文字列で下の AUTH_PIN_HASH を書き換えて push する。
//
// ■ デフォルト PIN: 1234
// ══════════════════════════════════════════════════════════════
...
// ── ロックアウト判定 ──
function _isLocked() {
  if (!_auth.lockedAt) return false;
  return (Date.now() - _auth.lockedAt) / 1000 < AUTH_LOCK_SEC;
}
function _lockRemain() {
  return Math.ceil(AUTH_LOCK_SEC - (Date.now() - _auth.lockedAt) / 1000);
}

// ── キーパッドの有効 / 無効 ──
function _setKeypadEnabled(on) {
  document.querySelectorAll('.pin-key').forEach(b => { b.disabled = !on; });
}
  ...
  ov.innerHTML = `
    <div class="pin-card">
      ...
      <div class="pin-keypad">
        <button class="pin-key" onclick="authKeyPress('1')">1</button>
        <button class="pin-key" onclick="authKeyPress('2')">2</button>
        <button class="pin-key" onclick="authKeyPress('3')">3</button>
        <button class="pin-key" onclick="authKeyPress('4')">4</button>
        <button class="pin-key" onclick="authKeyPress('5')">5</button>
        <button class="pin-key" onclick="authKeyPress('6')">6</button>
        <button class="pin-key" onclick="authKeyPress('7')">7</button>
        <button class="pin-key" onclick="authKeyPress('8')">8</button>
        <button class="pin-key" onclick="authKeyPress('9')">9</button>
        <span class="pin-key-empty"></span>
        <button class="pin-key" onclick="authKeyPress('0')">0</button>
        <button class="pin-key pin-key-back" onclick="authBackspace()" aria-label="削除">
